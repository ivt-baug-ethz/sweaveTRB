...
Manuscript line numbering is implemented in the \verb1trbunofficial.cls1 class document. For initial submission, use \verb1\documentclass[numbered]{trbunofficial}1 at the beginning of the \verb1.rnw1 or \verb1.tex1 file to show pagewise line numbers.  For final submission (if accepted for publication in Transportation Research Record), simply use \verb1\documentclass{trbunofficial}1 instead.  
